# 匯入Server 清單
$computers = get-Content .\computer-list-1.txt
...
# 將Server清單匯入
  foreach ($computer in $computers) {
     ...
     $objHostInfo | Add-Member -MemberType NoteProperty -Name Name -Value $computer.trim()
     $objHostInfo | Add-Member -MemberType NoteProperty -Name CPU% -Value $ComputerCpu
     $objHostInfo | Add-Member -MemberType NoteProperty -Name Memory% -Value $PercentMemoryUsed
     $a="";     
     if($UseDisk -is [array]){	
